...
export function WordGame() {
  ...
  useEffect(() => {
    ...
    const initialFontSize = [10, 12, 15, 18].includes(savedFontSize) ? savedFontSize : 12;
  ...
  return (
    <main className="app-shell">
      ...
      <section className="game-layout">
        ...
        <section className="game-card" aria-label="다중 단어 타자 연습 게임">
          <div className="scorebar">
            ...
            <div className="score-font-control" aria-label="글자 크기 빠른 선택">
              <small>FONT</small>
              <div>{[10, 12, 15, 18].map((size) => <button type="button" key={size} className={wordFontSize === size ? "active" : ""} onClick={() => changeWordFontSize(size)}>{size}</button>)}</div>
            </div>
          ...
          <div className="fall-zone">
            ...
            {fallingWords.map((item) => (
              ...
              >
                {item.burst && <span className="burst" aria-hidden="true">{Array.from({ length: 16 }).map((_, index) => <i key={index} />)}</span>}
      ...
      <footer className="footer-note"><span>CopyRight HanQube Solution</span><i /><a href="mailto:callor@callor.com">callor@callor.com</a><b>Callor Word Master v0.1.5</b></footer>
    </main>
  );
}
